...
function _getShipments() {
  // Intentar todas las formas posibles de acceder al estado
  if (window.S && window.S.shipments) return window.S.shipments;
  if (typeof S !== 'undefined' && S && S.shipments) return S.shipments;
  // Buscar en variables globales del panel
  for (var key in window) {
    try {
      if (window[key] && window[key].shipments && Array.isArray(window[key].shipments)) {
        return window[key].shipments;
      }
    } catch(e) {}
  }
  return null;
...
function _findShip(shipId) {
  ...
  if (!list) return null;
  return list.find(function(x){ return x.id === shipId; }) || null;
}
...
Tracking.init = function() {
  _injectCSS();
  _injectOverlays();
  // Exponer S globalmente si no está en window
  // (el panel lo tiene como variable local)
  setTimeout(function() {
    if (typeof S !== 'undefined' && S && !window.S) {
      window.S = S;
      console.log('[Tracking] window.S sincronizado:', S.shipments && S.shipments.length, 'pedidos');
    }
    autoTrackingCheck();
  }, 3000);
