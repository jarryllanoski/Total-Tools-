...
async function autoTrackingCheck() {
  ...
  var pendientes = _ships.filter(function(s) {
    ...
    // Intervalo según etiqueta:
    // LLEGÓ A DESTINO / PENDIENTE DE PAGO → cada 2h (está cerca, puede entregar pronto)
    // ENVIADO y otros → cada 6h (está en camino, no hay urgencia)
    var intervalo = (s.status === 'LLEGÓ A DESTINO' || s.status === 'PENDIENTE DE PAGO')
      ? 2 * 60 * 60 * 1000   // 2 horas
      : TRK.AUTO_INTERVAL_MS; // 6 horas
