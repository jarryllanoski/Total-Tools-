...
async function handleCreate(req, res) {
  ...
  if (!order.name || !order.phone) {
    res.status(400).json({ status: 'error', error: 'Nombre y teléfono requeridos' });
    return;
  }
...
async function handleToken(req, res) {
  ...
  if (!tokenId) { res.json({ status: 'invalid' }); return; }

  const snap = await db.doc(`${TOK_COL}/${tokenId}`).get();
  if (!snap.exists) { res.json({ status: 'invalid' }); return; }
