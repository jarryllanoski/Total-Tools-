// ============================================================
// app.js — Lógica principal del Panel de Envíos
// Edita este archivo para agregar funciones, botones, lógica
// ============================================================

import { fbSave, fbSaveShipment, fbDeleteShipment, fbLoad, fbGetConfig } from "./firebase.js";
...
const _initFirebase = async () => {
  try {
    window._fbStatus && window._fbStatus("loading");
    ...
    // Start polling for changes every 5s
    setInterval(async () => {
      try {
        const cfg = await fbGetConfig();
        if(!cfg) return;
        const remoteTs = cfg.ts || 0;
        if(remoteTs > (window._S_TS || 0) + 3000) {
          const r = await fbLoad();
          if(r && window._mergeRemote) window._mergeRemote(r);
        }
      } catch(e) {}
    }, 5000);
  } catch(e) {
...
function openDel(id){
  ...
  $('delYes').onclick=()=>{
    ...
    if(window._fbDeleteShipment) fbDeleteShipment(id);
...
function delSelected(){
  ...
  $('delYes').onclick=()=>{
    sel.forEach(s=>{
      S.trash.push({shipment:s, deletedAt:Date.now()});
      // Borrar de Firestore inmediatamente
      if(window._fbDeleteShipment) fbDeleteShipment(s.id);
    });
