      ...
      cards.push(`
        <div class="card">
          ...
          <div class="destinatario">
            <div class="dest-label">DESTINATARIO${bLabel}</div>
            <div class="dest-name">${esc(s.name)}</div>
            <div class="dest-phone">📞 ${esc(s.phone)}</div>
            ${s.dni?`<div class="dest-ref">🪪 DNI: ${esc(s.dni)}</div>`:''}
            <div class="dest-addr">📍 ${esc(addr)}</div>
            ${s.referencia?`<div class="dest-ref">🔖 ${esc(s.referencia)}</div>`:''}
            ${s.dniRecoger?`<div class="dest-ref">🪪 DNI recoger: ${esc(s.dniRecoger)}</div>`:''}
            ${notes}
          </div>
...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  /* Escala inteligente: usa todo el espacio disponible */
  html, body { width:100%; height:100%; font-family:'Arial',sans-serif; background:#fff; }
  body { padding:4mm; display:flex; flex-direction:column; }
  .header { text-align:center; margin-bottom:4mm; padding-bottom:3mm; border-bottom:2px solid #000; flex-shrink:0; }
  .header h1 { font-size:clamp(11pt,2.5vw,16pt); font-weight:900; letter-spacing:1px; }
  .header p  { font-size:clamp(8pt,1.5vw,10pt); color:#555; margin-top:1mm; }
  /* Grid flexible — 1 columna si es sticker, 2 si es A4 */
  .grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(80mm,1fr)); gap:4mm; flex:1; align-content:start; }
  .card {
    border:2px solid #000; border-radius:5px; padding:3mm;
    page-break-inside:avoid; break-inside:avoid;
    display:flex; flex-direction:column; justify-content:space-between;
    background:#fff;
  }
  .card-top { display:flex; justify-content:space-between; align-items:flex-start; gap:3mm; margin-bottom:2mm; }
  .remitente { flex:1; min-width:0; }
  .rem-label { font-size:clamp(6pt,1.2vw,8pt); font-weight:700; color:#888; letter-spacing:1px; text-transform:uppercase; margin-bottom:1mm; }
  .rem-name  { font-size:clamp(8pt,1.8vw,11pt); font-weight:700; line-height:1.2; }
  .rem-sub   { font-size:clamp(7pt,1.4vw,9pt); color:#555; margin-top:0.5mm; }
  .qr-box    { text-align:center; flex-shrink:0; }
  .qr-box img { display:block; width:clamp(18mm,12vw,28mm); height:clamp(18mm,12vw,28mm); }
  .qr-label  { font-size:clamp(6pt,1vw,7pt); color:#555; margin-top:0.5mm; }
  .divider   { border-top:1.5px dashed #999; margin:2mm 0; }
  .destinatario { flex:1; }
  .dest-label { font-size:clamp(6pt,1.2vw,8pt); font-weight:700; color:#888; letter-spacing:1px; text-transform:uppercase; margin-bottom:1mm; }
  .dest-name  { font-size:clamp(13pt,3vw,18pt); font-weight:900; margin-bottom:1.5mm; line-height:1.15; }
  .dest-phone { font-size:clamp(10pt,2.2vw,13pt); font-weight:700; color:#000; margin-bottom:1mm; }
  .dest-addr  { font-size:clamp(8pt,1.6vw,10pt); color:#333; margin-bottom:1mm; line-height:1.4; }
  .dest-ref   { font-size:clamp(7pt,1.4vw,9pt); color:#555; margin-bottom:0.5mm; }
  .agencia    { font-size:clamp(7pt,1.4vw,9pt); color:#333; margin-bottom:0.5mm; }
  .notes      { font-size:clamp(6pt,1.2vw,8pt); color:#777; font-style:italic; margin-top:1mm; }
  .card-footer { display:flex; justify-content:space-between; align-items:center; margin-top:2mm; padding-top:2mm; border-top:1px solid #ddd; flex-shrink:0; }
  .courier { font-size:clamp(7pt,1.4vw,9pt); font-weight:700; background:#f3f4f6; padding:0.5mm 2mm; border-radius:3px; }
  .fecha   { font-size:clamp(7pt,1.4vw,9pt); color:#555; }
  .codigo  { font-size:clamp(9pt,2vw,12pt); font-weight:900; color:#000; letter-spacing:1px; }
  .card-thanks { margin-top:1.5mm; padding-top:1.5mm; border-top:1px dashed #ccc; display:flex; justify-content:space-between; align-items:center; flex-shrink:0; }
  .card-thanks-txt { font-size:clamp(6pt,1.1vw,7.5pt); color:#888; font-style:italic; }
  .card-thanks-time { font-size:clamp(6pt,1.1vw,7pt); color:#aaa; }
  @media print {
