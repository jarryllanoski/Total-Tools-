...
function _getDeliveryShipments(){
  ...
  return ships.filter(function(s){
    return s.courier && s.courier.toUpperCase().includes('DELIVERY');
  });
}
...
function _attachTapListeners(){
  // Delegación de eventos en document — funciona aunque los cards se re-rendericen
  document.addEventListener('click', function(e){
    var hdr = e.target.closest('.cgroup-hdr');
    if(!hdr) return;
    var name = hdr.querySelector('.cgroup-name');
    if(!name) return;
    if(!name.textContent.trim().toUpperCase().includes('DELIVERY')) return;
  ...
  console.log('[DeliveryModule] Triple-tap listener activo en document');
}
...
DeliveryModule.abrir = function(){
  _loadDrivers();
  var ov = document.getElementById('dlvOv');
  if(!ov){ console.error('[Delivery] Overlay no encontrado'); return; }
  ov.classList.add('open');
  console.log('[Delivery] Panel abierto —', _getDeliveryShipments().length, 'pedidos DELIVERY');
  _renderList();
};
