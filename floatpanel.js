...
(function(){
  ...
  function createPanel(label, icon){
    ...
    // Resize
    _makeResizable(el, el.querySelector('.fp-resize'));
  ...
  function _makeResizable(panel, handle){
    ...
    document.addEventListener('mousemove', function(e){
      if(!resizing) return;
      var nw = Math.max(220, Math.min(600, sw + e.clientX - sx));
      var nh = Math.max(120, Math.min(window.innerHeight * 0.85, sh + e.clientY - sy));
